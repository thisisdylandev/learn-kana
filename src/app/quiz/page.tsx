...
function Quiz() {
  ...
  columns.forEach(item => {
    kana.forEach(char => {if (char.romanization.startsWith(item)) kanaQuiz.push(char) });
  })
  if (columns.includes("variants")) {
    kana.forEach(char => {
      if (char.romanization.startsWith('d') || char.romanization.startsWith('g') || char.romanization.startsWith('z') || char.romanization.startsWith('j') || char.romanization.startsWith('b') || char.romanization.startsWith('p')) {
        kanaQuiz.push(char)
      }
    });
  }
  ...
  if (columns.includes("combo")) { kana.forEach(char => {if (char.character.length >= 2) kanaQuiz.push(char) });}
  ...
  const updateKana = (e: React.FormEvent) => {
    ...
    testArray.shift()
    if (testArray.length === 0){ router.push(`/?correct=${correct.toString()}&incorrect=${incorrect.toString()}&remaining=0`) }
